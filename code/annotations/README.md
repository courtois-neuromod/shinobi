...
This dataset: **666 repetitions**, **244,032 event rows**, **24
...
| `Player_state/Hit_recovery` (durational) | *none: requires the `hit_timer` variable, present only in sidecars replayed with the current `data.json`* | Post-hit recovery: the player has just been damaged and blinks. In the Mario games nothing can hurt the player until it ends; in Shinobi it is the game's post-hit counter. |
